...
1. Критериях и требованиях для получения учёного звания согласно Постановлению Правительства РФ №1139
...
Основные критерии (Постановление РФ №1139):
...
При ответах используй информацию из этих фрагментов в первую очередь.`;
  } catch {
    return buildSystemPromptFallback();
  }
}

async function selectRagChunks(userMessage: string, topK = 4) {
  const queryEmbedding = await generateEmbedding(userMessage);
  const similarity = sql<number>`1 - (${cosineDistance(ragDocumentChunksTable.embedding, queryEmbedding)})`;
