...
try {
  ...
  // Не все сабпуты доступны из-за exports, поэтому вычисляем путь относительно `pdf-parse/worker`.
  // `require.resolve('pdf-parse/worker')` возвращает entry в `dist/worker/cjs/index.cjs`,
  // а worker-файл лежит в `dist/worker/pdf.worker.mjs`.
...
// Для PDF используем pdf-parse; для текстовых форматов читаем напрямую.
async function extractTextFromFile(filePath: string, _mimeType: string): Promise<string> {
  try {
    ...
    // Для DOC/DOCX сейчас возвращаем пустую строку (можно расширить позже)
    return "";
  } catch (e) {
    // eslint-disable-next-line no-console
...
router.get("/admin/rag-documents", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    ...
    // Если `rag_documents.content` был обнулён, но embeddings/чанки в pgvector сохранились,
    // восстанавливаем контент для UI-редактора по `chunkText`.
    res.json(
      docs.map((d) => {
        const reconstructed = chunksByDocId.get(d.id)?.join("\n\n") ?? "";
        const baseContent = d.content ?? "";
        const resolvedContent =
          baseContent.trim().length > 0 ? baseContent : reconstructed;
...
router.post("/admin/rag-documents/upload", requireAuth, requireRole("admin"), upload.single("file"), async (req, res) => {
  try {
    ...
    // Пытаемся извлечь текст из документа
    ...
    // Если есть текст — чанкуем и сохраняем embeddings в pgvector
    if (content && content.trim().length > 0) {
      const chunks = chunkText(content, { chunkSize: 1200, overlap: 200 }).slice(0, getMaxChunks());
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        try {
          ...
          // Важно: embeddings должны иметь нужную размерность.
          // Если модель вернула другое — pgvector insert упадет, и это будет видно в логах.
...
// Обновить содержимое документа вручную (для случаев, когда автоизвлечение не работает)
router.put("/admin/rag-documents/:id/content", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    ...
    // Если контент менялся — пересобираем pgvector чанки (best-effort).
    if (updates.content !== undefined) {
      const newContent = typeof content === "string" ? content : updated.content;
      if (newContent && newContent.trim().length > 0) {
        // Только когда контент реально непустой — пересоздаём чанки.
        // Если контент пустой (случайно), не удаляем embeddings, чтобы не ломать RAG.
        await db
          .delete(ragDocumentChunksTable)
          .where(eq(ragDocumentChunksTable.documentId, id));

        const chunks = chunkText(newContent, { chunkSize: 1200, overlap: 200 }).slice(0, getMaxChunks());
...
router.delete("/admin/rag-documents/:id", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    ...
    } catch { /* Если файл не найден — не страшно */ }
