...
export default function Apply() {
  ...
  const nextStep = async () => {
    // Валидация текущих полей перед переходом
    if (step === 1) {
      if (!rankType) {
        alert('Пожалуйста, выберите учёное звание');
        return;
      }
    }
    if (step === 2) {
      const isValid = await trigger(['academicExperienceYears', 'pedagogicalExperienceYears']);
      if (!isValid) {
        alert('Пожалуйста, заполните оба поля стажа');
        return;
      }
    }
    if (step === 3) {
      const isValid = await trigger(['publicationsCount', 'textbooksCount', 'scopusWosCount']);
      if (!isValid) {
        alert('Пожалуйста, заполните все поля публикаций');
        return;
      }
    }
    // Всегда переходим на следующий шаг, включая шаг 4
    setStep(s => {
      const next = s + 1;
      console.log(`[STEP] Переход с шага ${s} на шаг ${next}`);
          ...
          <div onKeyDown={(e) => { 
            ...
            <div className="flex justify-between mt-10 pt-6 border-t border-slate-100">
              ...
              {step < 4 ? (
                ...
                <Button type="button" onClick={() => handleSubmit(onSubmit)()} isLoading={createMutation.isPending} className="bg-success hover:bg-success/90 shadow-success/20">
